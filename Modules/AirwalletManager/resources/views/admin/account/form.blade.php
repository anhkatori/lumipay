...
@section('content')
    ...
    <div class="card mx-3 mt-3">
        ...
        <form action="{{ isset($airwalletAccount) ? route('admin.airwallet-accounts.update', $airwalletAccount->id) : route('admin.airwallet-accounts.store') }}" method="POST">
            <div class="card-body p-5">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control @error('domain') is-invalid @enderror" id="domain" name="domain" value="{{ old('domain', isset($airwalletAccount) ? $airwalletAccount->domain : '') }}" >
                </div>

                <div class="form-group mb-3">
                    <label class="form-label" for="max_receive_amount">Max Receive Amount</label>
                    <input type="number" step="0.01" class="form-control @error('max_receive_amount') is-invalid @enderror" id="max_receive_amount" name="max_receive_amount" value="{{ old('max_receive_amount', isset($airwalletAccount) ? $airwalletAccount->max_receive_amount : '') }}" >
                </div>

                <div class="form-group mb-3">
                    <label class="form-label" for="current_amount">Current Amount</label>
                    <input type="number" step="0.01" class="form-control @error('current_amount') is-invalid @enderror" id="current_amount" name="current_amount" value="{{ old('current_amount', isset($airwalletAccount) ? $airwalletAccount->current_amount : '') }}" >
                </div>
            ...
            <div class="card-footer p-3 px-5">
                <button type="submit" class="float-end btn btn-primary">{{ isset($airwalletAccount) ? 'Update Account' : 'Create Account' }}</button>
            </div>
        </form>
    </div>
@endsection
